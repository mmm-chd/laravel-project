...
<section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5 antialiased">
    <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
        ...
        @php
            $createModalId = 'createTeacherModal';
        @endphp
        @endforeach
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3 max-w-[12rem] truncate">{{ $teacher->address }}</td>
                            <td class="px-4 py-3 flex items-center justify-end">
                                @php
                                    $dropdownId = 'teacher-dropdown-' . $teacher->id;
                                    $buttonId = $dropdownId . '-button';
                                    $updateModalId = 'updateTeacherModal-' . $teacher->id;
                                    $viewModalId = 'viewTeacherModal-' . $teacher->id;
                                    $deleteModalId = 'deleteTeacherModal-' . $teacher->id;
                                @endphp
                        ...
                        <div id="{{ $createModalId }}" tabindex="-1" aria-hidden="true"
                          ...
                          <div class="relative p-4 w-full max-w-2xl max-h-full">
                            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                              ...
                              {{-- Isi form --}}
                              @include('admin.teachers.form-modal', ['subjects' => $subjects])
                            </div>
                          </div>
                        </div>
                        ...
                        <div id="{{ $updateModalId }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-2xl max-h-full">
                                ...
                                <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                    <!-- Modal header -->
                                    <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Update {{ $teacher->name}}</h3>
                                    ...
                                    {{-- Update Modal Body --}}
                                    @include('admin.teachers.update-modal')
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="{{ $viewModalId }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-xl max-h-full">
                                ...
                                <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                    {{-- Review Modal --}}
                                    @include('admin.teachers.read-modal')
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="{{ $deleteModalId }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                ...
                                <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                    ...
                                    {{-- Modal Content --}}
                                    @include('admin.teachers.delete-modal')
                                </div>
                            </div>
                        </div>
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    Showing
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $teachers->firstItem() }}</span>
                    to
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $teachers->lastItem() }}</span>
                    of
